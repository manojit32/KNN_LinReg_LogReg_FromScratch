...
def csv_to_point_label(csv_file, predictor_cols, label_col):
    ...
    with open(csv_file,'r') as input_file:
        ...
        for row in reader:
            row=row[0].split(" ")
            predictors = [float(row[i+1]) for i in predictor_cols]
...
def run(m):   
    data = csv_to_point_label("RobotDataset/Robot1", [1,2,3,4,5,6], 0)
    ...
    plt.show()'''
    print("Using Scikit-learn KNN Algorithm ::")
    import pandas as pd
    df=pd.read_csv("RobotDataset/Robot1",sep=" ")
